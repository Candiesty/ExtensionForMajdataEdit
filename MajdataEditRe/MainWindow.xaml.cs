...
#region MENU BARS
        ...
        private void Menu_Open_Click(object sender, RoutedEventArgs e)
        {
            ...
            if ((bool)openFileDialog.ShowDialog())
            {
                sendMp3Request(0,openFileDialog.FileName);
                FileInfo fileInfo = new FileInfo(openFileDialog.FileName);
                initFromFile(fileInfo.DirectoryName);
            }
        }
        ...
        private void sendMp3Request(int mode,double start, double end, string note)
        {
            try
            {
                string request = mode.ToString() + ";";
                request += start.ToString() + ";";
                request += end.ToString() + ";";
                request += startBpm + ";";
                request += startSignature + ";";
                request += "&title=" + SimaiProcess.title + "\n";
                request += "&artist=" + SimaiProcess.artist + "\n";
                request += "&first=" + SimaiProcess.first + "\n";
                request += "&des=" + SimaiProcess.designer + "\n";
                request += SimaiProcess.other_commands + "\n;";
                request += note;
                var response = WebControl.RequestPOST("http://localhost:23333/", request);
            }
            catch
            {
                MessageBox.Show("发送Mp3请求失败");
            }
            return;
        }
        private void sendMp3Request(int mode,string root)
        {
            try
            {
                string request = mode.ToString() + ";";
        ...
        private void Captur_MenuItem_Click(object sender, RoutedEventArgs e)
        {
            var CusorTime = SimaiProcess.Serialize(GetRawFumenText(), GetRawFumenPosition());
            if (captureToSave)
            {
                var text = GetRawFumenText();
                var pos = (int)GetRawFumenPosition();
                var capTime = Math.Round(SimaiProcess.Serialize(text, pos),3);
                text = text.Substring(lastCapturePosition,pos-lastCapturePosition);
                MessageBox.Show(text);
                MessageBox.Show(lastCaptureTime.ToString()+" "+capTime.ToString());
                captureToSave = false;
                sendMp3Request(1,lastCaptureTime, capTime, text);
            }
            else
            {
                captureToSave = true;
                lastCapturePosition = (int)GetRawFumenPosition();
                lastCaptureTime = Math.Round(SimaiProcess.Serialize(GetRawFumenText(), lastCapturePosition),3);//scan first
                var temp = SimaiProcess.GetBeatsAndBpm(GetRawFumenText(), lastCapturePosition);
                startBpm = (float)temp[0];
                startSignature = (int)temp[1];
                Console.WriteLine(startSignature);
                Console.WriteLine(startBpm);
                MessageBox.Show("截取位置获取成功");
            }
        }
...
#region 快捷键
        ...
        private void Capture_CanExecute(object sender, CanExecuteRoutedEventArgs e)
        {
            Captur_MenuItem_Click(sender, null);
        }
        #endregion
